...
from ml.paths import REPO_ROOT, SHARKS_CSV
...
def check_data_files():
    ...
    required_files = {
        'shark_data': str(SHARKS_CSV),
        'nasa_sst': str(REPO_ROOT / 'nasa_data/modis_aqua/modis_aqua_sst.nc'),
        'nasa_chlorophyll': str(REPO_ROOT / 'nasa_data/modis_aqua/modis_aqua_chlorophyll.nc'),
        'nasa_terra_sst': str(REPO_ROOT / 'nasa_data/modis_terra/modis_terra_sst.nc'),
    }
...
def run_preprocessing_pipeline():
    ...
    try:
        ...
        nasa_data_paths = {
            'sst': str(REPO_ROOT / 'nasa_data/modis_aqua/modis_aqua_sst.nc'),
            'chlorophyll': str(REPO_ROOT / 'nasa_data/modis_aqua/modis_aqua_chlorophyll.nc'),
            'terra_sst': str(REPO_ROOT / 'nasa_data/modis_terra/modis_terra_sst.nc'),
        }
        ...
        integrated_data = integrator.integrate_all_data(
            shark_data_path=str(SHARKS_CSV),
...
def main():
    ...
    if not SHARKS_CSV.is_file():
        logger.error(
            "Missing %s — add shark tracks under data/ (see data/README.md). "
            "From the repository root run: python -m ml.run_data_preprocessing",
            SHARKS_CSV,
        )
        sys.exit(1)
