"""
Scale Data Pipeline to Full Dataset (65,793 records)
Uses clean, leakage-free approach validated on small dataset
"""
...
from ml.paths import INTEGRATED_DATA_FULL_CSV, SHARKS_CSV
...
class FullDatasetProcessor:
    """
    Process the full shark dataset using the clean, leakage-free approach
    """
    
    def __init__(self):
        self.scaler = StandardScaler()
        
    def load_full_shark_data(self, file_path: Optional[str] = None) -> pd.DataFrame:
        """Load the full shark dataset"""
        path = file_path or str(SHARKS_CSV)
        logger.info(f"Loading full shark dataset from {path}")
        ...
        logger.info(f"Species: {df['species'].unique()}")
    ...
    def create_synthetic_satellite_data(self, shark_df: pd.DataFrame) -> pd.DataFrame:
        """
        Create synthetic satellite data for the full dataset
        This will be replaced with real NASA data later
        """
        logger.info("Creating synthetic satellite data for full dataset")
        ...
        logger.info("Synthetic satellite data created for full dataset")
        
        return df
    
    def create_proper_foraging_labels(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Create foraging labels using temporal patterns (leakage-free approach)
        """
...
class FullDatasetModel:
    ...
    def __init__(self):
        self.models = {
            ...
            'GradientBoosting': GradientBoostingClassifier(n_estimators=100, random_state=42)  # Fast and effective
        }
        self.trained_models = {}
        self.results = {}
    ...
    def create_visualizations(self, X_test, y_test, output_dir: str = "results_full"):
        ...
        plt.title('ROC Curves - Full Dataset Model (No Data Leakage)')
        plt.legend()
        plt.grid(True)
        plt.savefig(f"{output_dir}/roc_curves_full.png", dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(f"{output_dir}/confusion_matrices_full.png", dpi=300, bbox_inches='tight')
        plt.close()
        ...
        if 'RandomForest' in self.trained_models:
            ...
            plt.title('Top 15 Most Important Features (Full Dataset)')
            plt.gca().invert_yaxis()
            plt.tight_layout()
            plt.savefig(f"{output_dir}/feature_importance_full.png", dpi=300, bbox_inches='tight')
            plt.close()
        
        logger.info(f"Visualizations saved to {output_dir}/")
    
    def save_results(self, X_test, y_test, output_dir: str = "results_full"):
        ...
        for name, model in self.trained_models.items():
            model_path = f"{models_dir}/{name.lower()}_model.pkl"
        ...
        with open(f"{output_dir}/model_performance_full.json", 'w') as f:
        ...
        predictions_df.to_csv(f"{output_dir}/predictions_full.csv", index=False)
    ...
    def print_summary(self):
        """Print summary"""
        print("\n" + "="*60)
        print("FULL DATASET MODEL RESULTS (NO DATA LEAKAGE)")
        print("="*60)
...
def main():
    """Main function to scale to full dataset"""
    ...
    df = processor.load_full_shark_data()
    
    # Create synthetic satellite data (will be replaced with real NASA data later)
    df = processor.create_synthetic_satellite_data(df)
    
    # Create proper foraging labels (temporal-based, not spatial clustering)
    df = processor.create_proper_foraging_labels(df)
    
    # Create leakage-free features
    df = processor.create_leakage_free_features(df)
    
    # Save full processed data
    INTEGRATED_DATA_FULL_CSV.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(INTEGRATED_DATA_FULL_CSV, index=False)
    logger.info("Saved full processed data to %s", INTEGRATED_DATA_FULL_CSV)
    
    # Prepare training data with temporal splitting
    prepared_data = processor.prepare_training_data(df)
    
    # Initialize and train model
    model = FullDatasetModel()
    ...
    print(f"1. ✅ Processed {len(df):,} records (vs 436 in test)")
    print(f"2. ✅ Used clean, leakage-free approach")
    print(f"3. ✅ Implemented temporal train/test split")
    print(f"4. ✅ Trained models on full dataset")
    print(f"5. ✅ Generated comprehensive results")
    
    print("\nGenerated files:")
    print(f"  - {INTEGRATED_DATA_FULL_CSV} (Full processed data)")
    print("  - results_full/model_performance_full.json (Full dataset performance)")
    print("  - results_full/predictions_full.csv (Full dataset predictions)")
    print("  - results_full/roc_curves_full.png (Full dataset ROC curves)")
    print("  - results_full/confusion_matrices_full.png (Full dataset confusion matrices)")
    print("  - results_full/feature_importance_full.png (Full dataset feature importance)")
    
    print("\nNext steps:")
    print("  1. 🔄 Replace synthetic data with real NASA MODIS data")
    print("  2. 🔄 Implement hyperparameter tuning")
    print("  3. 🔄 Add ensemble methods")
    print("  4. 🔄 Create production prediction API")
